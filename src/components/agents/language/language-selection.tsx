    ...
    <div className="space-y-10 duration-500 animate-in fade-in slide-in-from-bottom-4">
      <div className="text-center">
        ...
        <p className="text-[15px] font-medium leading-relaxed tracking-tight text-primary-100/60">
          In which tongue do you perceive the world?
        </p>
      </div>
        ...
        {filteredLanguages.length > 0 ? (
          filteredLanguages.map((language) => (
            <Card
              ...
              onClick={() => {
                updateConfig('nativeLanguage', language.value);
                onSelection?.(language.value);
              }}
            >
              {/* Gold side-accent on hover */}
              <div
                className={`absolute bottom-0 left-0 top-0 w-1 bg-primary-500 shadow-[2px_0_15px_rgba(212,175,55,0.4)] transition-opacity duration-300 group-hover:opacity-100 ${config.nativeLanguage === language.value ? 'opacity-100' : 'opacity-0'}`}
              />
