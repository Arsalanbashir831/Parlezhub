...
import { Service, ServiceFormData, ServiceType, ServiceStatus, ServiceFilters, ServiceStats } from "@/types/service";
import {
	loadServices,
	createService,
	updateService,
...
interface UseServicesReturn {
	...
	updateExistingService: (serviceId: string, updates: Partial<ServiceFormData>) => Promise<Service | null>;
	deleteExistingService: (serviceId: string) => Promise<boolean>;
	toggleServiceStatus: (serviceId: string, status: ServiceStatus) => Promise<Service | null>;
...
export function useServices(): UseServicesReturn {
	...
	// Create new service
	const createNewService = useCallback(async (formData: ServiceFormData): Promise<Service> => {
		try {
			setIsLoading(true);
	...
	const updateExistingService = useCallback(async (serviceId: string, updates: Partial<ServiceFormData>): Promise<Service | null> => {
		try {
			setIsLoading(true);
	...
	// Delete service
	const deleteExistingService = useCallback(async (serviceId: string): Promise<boolean> => {
		try {
			setIsLoading(true);
	...
	const toggleServiceStatus = useCallback(async (serviceId: string, status: ServiceStatus): Promise<Service | null> => {
		try {
	...
	const stats = useMemo((): ServiceStats => {
		...
		const totalSessions = services.reduce((sum, s) => sum + (s.totalSessions || 0), 0);
		const totalEarnings = services.reduce((sum, s) => sum + ((s.totalSessions || 0) * s.price), 0);
		
		const servicesWithRatings = services.filter(s => s.averageRating && s.averageRating > 0);
		const averageRating = servicesWithRatings.length > 0 
			? servicesWithRatings.reduce((sum, s) => sum + (s.averageRating || 0), 0) / servicesWithRatings.length
			: 0;
