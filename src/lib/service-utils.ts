import { Service, ServiceFormData, ServiceType, ServiceStatus } from "@/types/service";
...
const mockServices: Service[] = [
	{
		...
		description: "Get personalized guidance on improving your business Spanish communication skills. I'll analyze your current level and provide a detailed roadmap for improvement.",
		shortDescription: "Personalized business Spanish consultation and improvement roadmap",
		price: 50,
		duration: 60,
...
export function createService(formData: ServiceFormData, teacherId: string): Service {
...
export function updateService(serviceId: string, updates: Partial<ServiceFormData>): Service | null {
	const services = loadServices();
...
export function canCreateServiceType(teacherId: string, type: ServiceType): boolean {
	const teacherServices = getServicesByTeacher(teacherId);
	return !teacherServices.some(service => service.type === type);
}

export function updateServiceStatus(serviceId: string, status: ServiceStatus): Service | null {
	const services = loadServices();
	const serviceIndex = services.findIndex(s => s.id === serviceId);
